...
LASSO_coefficients_plot <- get_LASSO_coefficients(LASSO_fit_all_lambda) %>%
  ...
  geom_line() +
  labs(x = "lambda (log10-scale)", y = "beta-hat",col = "term")+
  scale_x_log10()
...
The LASSO model shrunk 12 variables to zero. The follow table displays the variables that were not retained:
...
Bottome line: LASSO is less specific to the training data, but is more robust for use in outside datasets. LASSO helps prevent overfitting.
...
test$log10_GrLivArea = 1 # just to run function
test$y_hat_LASSO = 1
...
# To avoid error when applying model to test data
# REF https://stackoverflow.com/questions/29015282/predict-function-error-for-probabilities-in-glmnet
X = sparse.model.matrix(model_formula_2, data = combined)
