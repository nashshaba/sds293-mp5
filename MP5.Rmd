...
# get_LASSO_coefficients(LASSO_fit_all_lambda) %>%
...
#Output dataframe
my_lasso_rse = get_LASSO_coefficients(LASSO_fit_optimal)
...
The LASSO model shrunk 12 variables to zero. The follow table displays the variables that were retained:

```{r}
my_lasso_res = get_LASSO_coefficients(LASSO_fit_optimal)
my_lasso_res = my_lasso_res[my_lasso_rse$estimate>0,]
...
=======
>>>>>>> 9b66925dde88146b8669b05f66babdbb0ce6a85a
```{r}
# Train your model here:

# Add log transform GrLivArea
<<<<<<< HEAD
training$log10_GrLivArea = log10(training$GrLivArea) # just to run function
# test$SalePrice = 1
...
# Model formula using some categorical variables
#model_formula_2 <- "log_SalePrice ~ log10_GrLivArea + OverallQual + LotArea + HouseStyle + Neighborhood + SaleCondition + Condition1 + #OverallCond + YearBuilt + CentralAir + Fireplaces + WoodDeckSF + MSSubClass + LotFrontage +  
#OverallCond + YearRemodAdd + MasVnrArea + BsmtFinSF1 + 
#BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + FirstFlrSF + SecondFlrSF + LowQualFinSF + 
#BsmtFullBath + BsmtHalfBath + FullBath + HalfBath + BedroomAbvGr + 
#KitchenAbvGr + TotRmsAbvGrd + GarageYrBlt + GarageCars + GarageArea + 
#OpenPorchSF + EnclosedPorch + ThirdSsnPorch + ScreenPorch + PoolArea + 
#MiscVal + MoSold + YrSold" %>%
#  as.formula()

model_formula_2 <- "log_SalePrice ~ log10_GrLivArea + OverallQual + LotArea + OverallCond + YearBuilt + CentralAir + Fireplaces + WoodDeckSF" %>% as.formula()
...
# The lambda_star is in the top row. 
lambda_star <- LASSO_CV_df$lambda[1]
lambda_star
